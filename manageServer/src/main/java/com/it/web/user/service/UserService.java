...
public class UserService {
     ...
     * 添加或保存用户扩展权限
     */
    public static Tb_UserAuthExtend saveOrUpdateUserAuthExtend(Tb_User mySelf, Tb_UserAuthExtend userAuthExtend) throws Exception {
        if (!mySelf.getAuthList().contains("用户管理")) throw new Exception("您没有用户管理权限");
        UserDao dao = new UserDao();
        return dao.saveOrUpdateUserAuthExtend(userAuthExtend);
    }

    /**
     * 删除用户扩展权限
     */
    public static void deleteUserAuthExtend(Tb_User mySelf, Tb_UserAuthExtend userAuthExtend) throws Exception {
        if (!mySelf.getAuthList().contains("用户管理")) throw new Exception("您没有用户管理权限");
        UserDao dao = new UserDao();
        dao.deleteUserAuthExtend(userAuthExtend);
    }

    /**
     * 查询用户扩展权限
     */
    public static List<Tb_UserAuthExtend> listUserAuthExtends(Tb_User mySelf, Long userId) {
        UserDao dao = new UserDao();
        return dao.listUserAuthExtends(userId);
    }
    ...
    public static void regMac(String macId, String computerName, String loginName, String password) throws Exception {
        ...
        Tb_Computer cmp = new Tb_Computer(computerName, macId, user.getRole().equals("管理员"), user.getId(), null);
        session.save(cmp);
    }
